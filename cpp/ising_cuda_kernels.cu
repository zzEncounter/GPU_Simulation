...
__device__ auto matrix_column_ptr(Complex *matrix, std::size_t dim,
                                  std::size_t column) -> Complex * {
    return matrix + column * dim;
}

__device__ auto matrix_column_ptr(const Complex *matrix, std::size_t dim,
                                  std::size_t column) -> const Complex * {
    return matrix + column * dim;
}
...
__device__ void apply_op_to_column(const Complex *in, Complex *out,
                                   std::size_t dim, std::size_t num_qubits,
    ...
    case OpKind::RZ: {
        ...
        for (std::size_t index = static_cast<std::size_t>(threadIdx.x); index < dim;
            ...
            double phase = bit ? half_theta : -half_theta;
            if (inverse) {
                phase = -phase;
            }
            const Complex factor(std::cos(phase), std::sin(phase));
...
__global__ void build_block_prefix_matrices_kernel(const OpDesc *ops,
                                                   ...
                                                   Complex *workspace_b,
                                                   Complex *prefix_out) {
    const auto column = static_cast<std::size_t>(blockIdx.x);
    if (column >= dim) {
        return;
    }

    const auto matrix_stride = dim * dim;
    Complex *current = matrix_column_ptr(workspace_a, dim, column);
    Complex *next = matrix_column_ptr(workspace_b, dim, column);

    for (std::size_t row = static_cast<std::size_t>(threadIdx.x); row < dim;
         row += static_cast<std::size_t>(blockDim.x)) {
        const Complex value =
            (row == column) ? Complex(1.0, 0.0) : Complex(0.0, 0.0);
        current[row] = value;
        prefix_out[row + column * dim] = value;
    }
    __syncthreads();

    for (std::size_t gate_index = 0; gate_index < block_length; gate_index++) {
        apply_op_to_column(current, next, dim, num_qubits, ops[gate_index], false);
        __syncthreads();

        Complex *prefix_column =
            prefix_out + (gate_index + 1) * matrix_stride + column * dim;
        for (std::size_t row = static_cast<std::size_t>(threadIdx.x); row < dim;
             row += static_cast<std::size_t>(blockDim.x)) {
            prefix_column[row] = next[row];
        }
        __syncthreads();
...
__global__ void build_block_suffix_matrices_kernel(const OpDesc *ops,
                                                   std::size_t block_length,
                                                   std::size_t num_qubits,
                                                   std::size_t dim,
                                                   Complex *workspace_a,
    ...
    for (std::size_t row = static_cast<std::size_t>(threadIdx.x); row < dim;
        ...
        const Complex value =
            (row == column) ? Complex(1.0, 0.0) : Complex(0.0, 0.0);
        current[row] = value;
        suffix_out[(block_length - 1) * matrix_stride + row + column * dim] = value;
    }
    __syncthreads();

    for (std::size_t gate_index = block_length; gate_index-- > 1;) {
        apply_op_to_column(current, next, dim, num_qubits, ops[gate_index], true);
        __syncthreads();

        Complex *suffix_column =
            suffix_out + (gate_index - 1) * matrix_stride + column * dim;
        for (std::size_t row = static_cast<std::size_t>(threadIdx.x); row < dim;
             row += static_cast<std::size_t>(blockDim.x)) {
            suffix_column[row] = next[row];
        }
        __syncthreads();
...
__global__ void block_gradients_kernel(const OpDesc *ops,
                                       std::size_t block_length,
                                       const Complex *psi_before_batch,
                                       const Complex *lambda_after_batch,
                                       double *out,
                                       std::size_t vector_size) {
    __shared__ double theta_partial[THREADS];
    __shared__ double phi_partial[THREADS];

    const auto op_index = static_cast<std::size_t>(blockIdx.x);
    if (op_index >= block_length) {
        return;
    }

    const OpDesc op = ops[op_index];
    if (!op.is_parametric) {
        return;
    }

    const Complex *psi = psi_before_batch + op_index * vector_size;
    const Complex *lambda = lambda_after_batch + op_index * vector_size;
    double theta_sum = 0.0;
    double phi_sum = 0.0;

    switch (op.kind) {
    case OpKind::FusedRYRZ: {
        const auto pairs = vector_size / 2;
        const auto half_stride = std::size_t{1} << op.wire0;
        const double c = std::cos(op.theta0 * 0.5);
        const double s = std::sin(op.theta0 * 0.5);
        const double half_phi = op.theta1 * 0.5;
        ...
        for (std::size_t pair_index = static_cast<std::size_t>(threadIdx.x);
            ...
            const Complex a0 = psi[i0];
            const Complex a1 = psi[i1];
            const Complex l0 = lambda[i0];
            const Complex l1 = lambda[i1];

            const Complex dtheta0 =
                phase0 * (Complex(-0.5 * s, 0.0) * a0 + Complex(-0.5 * c, 0.0) * a1);
            const Complex dtheta1 =
                phase1 * (Complex(0.5 * c, 0.0) * a0 + Complex(-0.5 * s, 0.0) * a1);
            theta_sum +=
                (thrust::conj(l0) * dtheta0 + thrust::conj(l1) * dtheta1).real();
    ...
    case OpKind::RY: {
        ...
        for (std::size_t pair_index = static_cast<std::size_t>(threadIdx.x);
            ...
            theta_sum += (thrust::conj(l0) * d0 + thrust::conj(l1) * d1).real();
        }
        break;
    }
    case OpKind::RZ: {
        const double half_theta = op.theta0 * 0.5;
        for (std::size_t index = static_cast<std::size_t>(threadIdx.x); index < vector_size;
             index += static_cast<std::size_t>(blockDim.x)) {
            const bool bit = bit_is_set(index, op.wire0);
            const double phase = bit ? half_theta : -half_theta;
            const Complex factor(std::cos(phase), std::sin(phase));
            const Complex prefactor = bit ? Complex(0.0, 0.5) : Complex(0.0, -0.5);
            theta_sum += (thrust::conj(lambda[index]) * prefactor * factor * psi[index]).real();
        }
        break;
    }
    case OpKind::CNOT:
    ...
    if (threadIdx.x == 0) {
        out[op.param_index0] = 2.0 * theta_partial[0];
        if (op.kind == OpKind::FusedRYRZ) {
            out[op.param_index1] = 2.0 * phi_partial[0];
        }
    }
}
...
__global__ void build_block_matrices_kernel(const OpDesc *ops, std::size_t num_blocks,
                                            std::size_t block_size,
                                            std::size_t num_ops,
                                            std::size_t num_qubits,
                                            std::size_t dim, Complex *block_mats,
                                            Complex *scratch_mats) {
    const auto block_id = static_cast<std::size_t>(blockIdx.y);
    const auto column = static_cast<std::size_t>(blockIdx.x);
    if (block_id >= num_blocks || column >= dim) {
        return;
    }
    ...
    const auto mat_elements = dim * dim;
    Complex *base_mat = block_mats + block_id * mat_elements;
    Complex *scratch_mat = scratch_mats + block_id * mat_elements;
    Complex *current = matrix_column_ptr(base_mat, dim, column);
    Complex *next = matrix_column_ptr(scratch_mat, dim, column);

    for (std::size_t row = static_cast<std::size_t>(threadIdx.x); row < dim;
         row += static_cast<std::size_t>(blockDim.x)) {
        current[row] = (row == column) ? Complex(1.0, 0.0) : Complex(0.0, 0.0);
    }
    __syncthreads();

    for (std::size_t local_idx = 0; local_idx < block_length; local_idx++) {
        apply_op_to_column(current, next, dim, num_qubits,
    ...
    if (current != matrix_column_ptr(base_mat, dim, column)) {
        for (std::size_t row = static_cast<std::size_t>(threadIdx.x); row < dim;
             row += static_cast<std::size_t>(blockDim.x)) {
            matrix_column_ptr(base_mat, dim, column)[row] = current[row];
        }
    }
}
...
void launch_build_block_matrices(const OpDesc *ops, std::size_t num_blocks,
    ...
    dim3 grid(static_cast<unsigned int>(dim),
              static_cast<unsigned int>(num_blocks));
    build_block_matrices_kernel<<<grid, THREADS>>>(
        ops, num_blocks, block_size, num_ops, num_qubits, dim, block_mats,
        scratch_mats);
    check_cuda(cudaGetLastError(), "build_block_matrices_kernel");
    maybe_synchronize_cuda("build_block_matrices_kernel sync");
}
