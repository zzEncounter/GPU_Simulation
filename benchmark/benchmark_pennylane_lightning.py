"""Batch benchmark for the PennyLane Lightning-GPU adjoint baseline.
...
OUTPUT_CSV = Path(__file__).resolve().parent / "results" / "pennylane_lightning_gpu.csv"
OVERWRITE_OUTPUT = True

# First threshold >= qubits wins. Small states get more timing samples; large
# states get progressively fewer samples so a full sweep remains practical.
STEP_SCHEDULE = (
    (8, 20),
    (14, 10),
    (20, 5),
...
energy_and_grad = importlib.import_module(
            ...
            for qubits in QUBITS:
                ...
                try:
                    ...
                    print(
                        ...
                        f"gpu_peak={result.memory.gpu_peak_observed_mib} MiB",
                        flush=True,
                    )
                except Exception as exc:  # noqa: BLE001 - keep long sweeps recoverable.
