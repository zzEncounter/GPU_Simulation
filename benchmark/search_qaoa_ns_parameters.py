"""Run the staged, resumable kernel-parameter search for QAOA-NS."""
...
def candidates(qubits: tuple[int, ...], csv_path: Path):
    for q in qubits:
        base = baseline()
        # Forward shape affects both the RX mixer and tiled nonshared RZZ cost.
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path,
                        default=ROOT / "benchmark/results/qaoa_ns_parameter_search.csv")
    parser.add_argument("--summary", type=Path,
                        default=ROOT / "benchmark/results/qaoa_ns_parameter_search.json")
    args = parser.parse_args()
    run_search(circuit="qaoa-ns", candidates=candidates(args.qubits, args.output),
