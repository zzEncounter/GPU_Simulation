"""Run the staged, resumable kernel-parameter search for Data Re-uploading."""
...
from search_circuit_common import (
    LOOKUP_GRID, ORDINARY_GRID, REGISTER_GRID, ROOT, SHAPE_GRID, best_config,
    make_candidate, phase_plan, run_search, valid_shape,
)


def parse_qubits(value: str) -> tuple[int, ...]:
    result = tuple(dict.fromkeys(int(item) for item in value.split(",")))
    if not result or any(q < 4 or q > 30 or q % 2 for q in result):
        raise argparse.ArgumentTypeError("qubits must be even comma-separated values in 4..30")
    return result
...
def candidates(qubits: tuple[int, ...], csv_path: Path):
    for q in qubits:
        ...
        plans = ("", phase_plan(q, base["forward_threads"],
        ...
        plans = ("", phase_plan(q, base["backward_threads"],
        ...
        for bits in LOOKUP_GRID:
            yield make_candidate("lookup_bits", q, {**base, "lookup_bits": bits},
                                 forward_phase_plan=f_plan, backward_phase_plan=b_plan)
        base, f_plan, b_plan = best_config(csv_path, q, "lookup_bits", "median_ms", base)
        for threads in (64, 128, 256, 512):
            yield make_candidate("diagonal_threads", q,
                                 {**base, "diagonal_threads": threads},
                                 forward_phase_plan=f_plan, backward_phase_plan=b_plan)
        base, f_plan, b_plan = best_config(csv_path, q, "diagonal_threads", "median_ms", base)
        ...
        for legacy, warp_atomic in ((1, 0), (0, 0), (0, 1)):
            yield make_candidate("rotation_reduction", q, {
                **base, "legacy_reduction": legacy,
                "rotation_warp_atomic": warp_atomic},
                forward_phase_plan=f_plan, backward_phase_plan=b_plan)
        base, f_plan, b_plan = best_config(csv_path, q, "rotation_reduction", "median_ms", base)
        for warp_atomic in (0, 1):
            yield make_candidate("diagonal_reduction", q,
                                 {**base, "diagonal_warp_atomic": warp_atomic},
                                 forward_phase_plan=f_plan, backward_phase_plan=b_plan)
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path,
                        default=ROOT / "benchmark/results/data_reuploading_parameter_search.csv")
    parser.add_argument("--summary", type=Path,
                        default=ROOT / "benchmark/results/data_reuploading_parameter_search.json")
    args = parser.parse_args()
    run_search(circuit="data-reuploading",
