...
from ring_ising.backends.standalone import StandaloneBackendConfig
from ring_ising.config import STANDALONE_GRADIENT_STRATEGIES
try:
    from ring_ising.workflows.pennylane import run_pennylane as _run_pennylane
    _PENNYLANE_AVAILABLE = True
except ImportError:
    _PENNYLANE_AVAILABLE = False
    _run_pennylane = None  # type: ignore[assignment]
...
# Extra virtual method aliases handled by this benchmark script
VIRTUAL_METHODS = ("structured_adjoint_db", "pennylane")
DEFAULT_METHODS = (
    "inverse_walk",
...
def parse_method(text: str) -> str:
    method = text.strip()
    if method == "reverse_walk":
        return "inverse_walk"
    # Allow virtual aliases handled by this script
    if method in VIRTUAL_METHODS:
        return method
    if method not in ALL_METHODS:
        choices = ", ".join((*ALL_METHODS, *VIRTUAL_METHODS, "reverse_walk"))
        raise argparse.ArgumentTypeError(
...
def resolve_method_config(method: str) -> tuple[str, str, bool]:
    """Return (backend, gradient_strategy, double_buffer) for a method alias."""
    if method == "structured_adjoint_db":
        return "standalone", "structured_adjoint", True
    if method == "pennylane":
        return "pennylane", "pennylane", False
    return "standalone", method, False
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--max-estimated-workspace-gib",
        type=float,
        default=None,
        help=(
            "Skip method/circuit runs whose estimated standalone backend workspace "
            "exceeds this GiB limit. Use this to avoid OOM."
        ),
    )
    parser.add_argument(
        "--kernel-profile-ncu",
...
# Cache of RunConfig field names (computed once at import time)
import inspect as _inspect
_RUN_CONFIG_FIELDS: frozenset[str] = frozenset(
    _inspect.signature(RunConfig.__init__).parameters.keys()
)


def _build_run_config_kwargs(
    backend: str,
    ...
    *,
    ...
    """Return only the kwargs that RunConfig.__init__ actually accepts."""
    # Base kwargs that exist in all versions
    ...
    # Optional fields – only pass if present in this version of RunConfig
    if "double_buffer" in _RUN_CONFIG_FIELDS:
        kwargs["double_buffer"] = double_buffer
    if "checkpoint_interval_ops" in _RUN_CONFIG_FIELDS:
        kwargs["checkpoint_interval_ops"] = args.checkpoint_interval_ops
    if "intrablock_block_size" in _RUN_CONFIG_FIELDS:
        kwargs["intrablock_block_size"] = args.intrablock_block_size
    if "gate_fusion" in _RUN_CONFIG_FIELDS:
        kwargs["gate_fusion"] = args.gate_fusion
    return kwargs
...
def build_run_config(
    ...
    *,
    ...
    backend, gradient_strategy, double_buffer = resolve_method_config(method)
    # --double-buffer flag also applies to the plain structured_adjoint method
    if method == "structured_adjoint":
        double_buffer = getattr(args, "double_buffer", False)

    return RunConfig(
        **_build_run_config_kwargs(
            backend,
            gradient_strategy,
            double_buffer,
            case=case,
...
def estimate_workspace_gib(method: str, *, case: BenchmarkCase, args: argparse.Namespace) -> float:
    _, gradient_strategy, _ = resolve_method_config(method)
    ...
    config.validate()
    strategy = config.normalized_gradient_strategy
    # estimated_gradient_workspace_gib_for may or may not accept an interval arg
    import inspect
    sig = inspect.signature(config.estimated_gradient_workspace_gib_for)
    if len(sig.parameters) > 1:
        # Older API: (strategy, interval)
        interval = (
            config.resolve_checkpoint_interval_ops(strategy)
            if hasattr(config, "resolve_checkpoint_interval_ops")
            else None
        )
        return config.estimated_gradient_workspace_gib_for(strategy, interval)
    return config.estimated_gradient_workspace_gib_for(strategy)


def exceeds_workspace_limit(method: str, *, case: BenchmarkCase, args: argparse.Namespace) -> tuple[bool, float]:
    # PennyLane backend has no standalone workspace estimate
    if method == "pennylane":
        return False, 0.0
    estimated_gib = estimate_workspace_gib(method, case=case, args=args)
    limit = args.max_estimated_workspace_gib
    return limit is not None and estimated_gib > limit, estimated_gib


def run_method(method: str, config: RunConfig):
    if config.backend == "pennylane":
        if not _PENNYLANE_AVAILABLE or _run_pennylane is None:
            raise RuntimeError(
                "PennyLane backend is not available. "
                "Install pennylane and pennylane-lightning[gpu]."
            )
        return _run_pennylane(config)
...
def append_inverse_walk_speedups(rows: list[dict[str, Any]]) -> None:
    """Add total-wall-time speedups relative to inverse_walk, inverse_walk_cuQuantum,
    and pennylane for each circuit."""

    # Baseline 1: inverse_walk
    baselines_iw: dict[tuple[int, int], float] = {}
    for row in rows:
        if row.get("gradient_strategy") != "inverse_walk":
            continue
        total_wall_s = row.get("total_wall_s")
        if not isinstance(total_wall_s, (float, int)) or total_wall_s <= 0:
            continue
        key = (int(row["num_qubits"]), int(row["layers"]))
        baselines_iw[key] = float(total_wall_s)

    # Baseline 2: inverse_walk_cuQuantum
    baselines_cuq: dict[tuple[int, int], float] = {}
    for row in rows:
        if row.get("gradient_strategy") != "inverse_walk_cuQuantum":
            continue
        total_wall_s = row.get("total_wall_s")
        if not isinstance(total_wall_s, (float, int)) or total_wall_s <= 0:
            continue
        key = (int(row["num_qubits"]), int(row["layers"]))
        baselines_cuq[key] = float(total_wall_s)

    # Baseline 3: pennylane (virtual method alias)
    baselines_pl: dict[tuple[int, int], float] = {}
    for row in rows:
        if row.get("method") != "pennylane":
            continue
        total_wall_s = row.get("total_wall_s")
        if not isinstance(total_wall_s, (float, int)) or total_wall_s <= 0:
            continue
        key = (int(row["num_qubits"]), int(row["layers"]))
        baselines_pl[key] = float(total_wall_s)

    for row in rows:
        key = (int(row["num_qubits"]), int(row["layers"]))
        total_wall_s = row.get("total_wall_s")

        # speedup vs inverse_walk
        baseline_iw = baselines_iw.get(key)
        if (
            baseline_iw is None
            or not isinstance(total_wall_s, (float, int))
            or total_wall_s <= 0
        ):
            row["total_wall_speedup_vs_inverse_walk"] = None
        else:
            row["total_wall_speedup_vs_inverse_walk"] = baseline_iw / float(total_wall_s)

        # speedup vs inverse_walk_cuQuantum
        baseline_cuq = baselines_cuq.get(key)
        if (
            baseline_cuq is None
            or not isinstance(total_wall_s, (float, int))
            or total_wall_s <= 0
        ):
            row["total_wall_speedup_vs_inverse_walk_cuQuantum"] = None
        else:
            row["total_wall_speedup_vs_inverse_walk_cuQuantum"] = baseline_cuq / float(total_wall_s)

        # speedup vs pennylane
        baseline_pl = baselines_pl.get(key)
        if (
            baseline_pl is None
            or not isinstance(total_wall_s, (float, int))
            or total_wall_s <= 0
        ):
            row["total_wall_speedup_vs_pennylane"] = None
        else:
            row["total_wall_speedup_vs_pennylane"] = baseline_pl / float(total_wall_s)
...
def backend_phase_total_s(result: Any) -> float | None:
    metadata = getattr(result, "metadata", {}) or {}
    totals = metadata.get("backend_timing_totals_s", {})
    if not isinstance(totals, dict) or not totals:
        return None
    return sum(float(value) for value in totals.values())
...
def main() -> None:
    ...
    if args.max_estimated_workspace_gib is not None:
        print(f"  Max estimated workspace: {args.max_estimated_workspace_gib:.2f} GiB")
    ...
    for case in args.cases:
        ...
        for method in args.methods:
            ...
            skip_for_workspace, estimated_workspace_gib = exceeds_workspace_limit(
                method, case=case, args=args
            )
            if skip_for_workspace:
                limit = args.max_estimated_workspace_gib
                message = (
                    f"estimated workspace {estimated_workspace_gib:.3f} GiB exceeds "
                ...
                run_rows.append(
                    {
                        ...
                        "status": "skipped_workspace_limit",
                        ...
                        "estimated_workspace_gib": estimated_workspace_gib,
                    }
                )
                continue
            ...
            if result is not None:
                phase_total_s = backend_phase_total_s(result)
                run_row.update(
                    {
                ...
                if phase_total_s is not None:
                    run_row["total_wall_s"] = phase_total_s
                    run_row["avg_step_ms"] = (
                        phase_total_s * 1000.0 / total_steps if total_steps else None
                    )
            ...
            print(
                ...
                f"avg_step_ms={run_row.get('avg_step_ms', 'NA')}",
                flush=True,
            )

    append_inverse_walk_speedups(run_rows)
