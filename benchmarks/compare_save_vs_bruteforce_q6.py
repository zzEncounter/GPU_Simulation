"""Compare save_param_states vs bruteforce_parallel_q6 with timing breakdown and GPU telemetry."""
...
def _run_single_mode(
    ...
    if collect_dense_diagnostics and mode == "bruteforce_parallel_q6":
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--worker-mode",
        choices=("save_param_states", "bruteforce_parallel_q6"),
        default=None,
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Skip dense_scan_experiment breakdown fields for bruteforce mode.",
    )
    parser.add_argument(
        "--dense-diag-repeats",
        type=int,
        default=1,
        help="Repeats used for dense_scan_experiment timing breakdown (bruteforce mode only).",
    )
    parser.add_argument(
        "--dense-diag-warmup",
        type=int,
...
def main() -> None:
    ...
    print("Standalone modes: save_param_states vs bruteforce_parallel_q6")
    ...
    for case in args.cases:
        results = [
            ...
            _run_worker_subprocess("bruteforce_parallel_q6", case, args),
        ]
