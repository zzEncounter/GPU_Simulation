        ...
        for case in cases:
            ...
            speedup_reference = next(
                (
                    ...
                    if row["mode"] == "reverse_walk" and row["status"] == "ok"
                ),
                None,
            )
            speedup_ref_step_ms = (
            ...
            for row in case_rows:
                if row["status"] != "ok":
                    row["speedup_vs_reverse_walk"] = None
            ...
            for row in case_rows:
                ...
                speedup_text = format_optional_float(
                    row["speedup_vs_reverse_walk"], ".3f", fallback="nan"
                )
