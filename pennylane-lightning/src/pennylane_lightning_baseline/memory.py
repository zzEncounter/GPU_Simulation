...
def _host_rss_mib() -> float:
    try:
        ...
        # ru_maxrss is KiB on Linux, so this is a peak fallback rather than current RSS.
...
def _gpu_memory_from_nvml() -> float | None:
    ...
    # NVML can raise several driver/version-specific exception classes. Memory
    # instrumentation is optional and must never prevent the benchmark itself.
...
def take_memory_snapshot() -> MemorySnapshot:
    """Read current process GPU memory and current/peak host RSS."""
